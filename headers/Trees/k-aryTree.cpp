...
public:
    ...
    template <typename U>
    friend std::ostream& operator<<(std::ostream& os, const KaryTree<U>& tree) {
        tree.print(tree.root, 0);
        return os;
    }

    template <typename U>
    friend std::istream& operator>>(std::istream& is, KaryTree<U>& tree) {
        U data;
        is >> data;
        tree.insert(data);
        return is;
    ...
    template <typename U>
    friend bool operator==(const KaryTree<U>& lhs, const KaryTree<U>& rhs) {
