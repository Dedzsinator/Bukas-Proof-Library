...
template<typename T>
inline auto to_unsigned_or_bool(T value) -> std::enable_if_t<!std::is_pointer_v<T> && std::is_integral_v<T>, decltype(static_cast<std::make_unsigned_t<T>>(value))>
{
    return static_cast<std::make_unsigned_t<T>>(value) + (std::is_signed_v<T> ? (1ULL << (sizeof(T) * 8 - 1)) : 0);
}

// Overload for bool
inline bool to_unsigned_or_bool(bool value)
{
    return value;
}
...
template<typename T>
inline auto to_unsigned_or_bool(T value) -> std::enable_if_t<std::is_floating_point_v<T>, std::conditional_t<sizeof(T) == 4, std::uint32_t, std::uint64_t>>
{
    using UIntType = std::conditional_t<sizeof(T) == 4, std::uint32_t, std::uint64_t>;
    union {
        T f;
        UIntType u;
    } as_union = { value };
    
    UIntType sign_bit = -std::make_signed_t<UIntType>(as_union.u >> (sizeof(UIntType) * 8 - 1));
    return as_union.u ^ (sign_bit | (UIntType(1) << (sizeof(UIntType) * 8 - 1)));
}
...
int main() {
    ...
    std::cout << "Is sorted: " << (is_sorted(data.begin(), data.end()) ? "\033[1;32m[Yes]\033[0m" : "\033[1;31m[No]\033[0m") << std::endl;
